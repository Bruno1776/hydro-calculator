...
const CONVERSION_FACTORS_FROM_BASE = {
  ...
  power_kW: { 'kW': 1, 'W': 1000, 'hp (mecânico)': (1 / 745.7) * 1000 }, // Corrigido
...
const CalculationViewComponent: React.FC<CalculationViewProps> = ({
  ...
  const getConvertedResultValue = (resultKey: string, baseValue: number): string => {
    ...
    if (!targetUnit || typeof baseValue !== 'number') return baseValue.toString(); // Retorna valor base se não houver unidade alvo ou não for número

    let conversionCategoryKey: keyof typeof CONVERSION_FACTORS_FROM_BASE | null = null;

    // Mapeia a chave do resultado para a categoria de conversão correta
    ...
    else if (resultKey === 'Potência de Eixo (kW)') conversionCategoryKey = 'power_kW'; // Usar a baseValue que já está em kW
    ...
    // Retorna o valor base formatado se nenhuma conversão aplicável for encontrada
